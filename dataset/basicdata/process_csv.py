import pandas as pd

df = pd.read_csv("../SalesTransactions/SalesTransactions.csv",
                 encoding="utf-8",
                 dtype="unicode",
                 low_memory=False)
print(df)

def find_orders_within_range(df, minValue, maxValue):
    order_totals = df.groupby('OrderID').apply(
        lambda x: (x['UnitPrice'].astype(float) *
                   x['Quantity'].astype(float) *
                   (1 - x['Discount'].astype(float))).sum()
    )
    orders_within_range = order_totals[
        (order_totals >= minValue) & (order_totals <= maxValue)
    ]
    unique_orders = df[df['OrderID'].isin(orders_within_range.index)]['OrderID'] \
        .drop_duplicates().tolist()

    return unique_orders

minValue = float(input("Nhập giá trị min: "))
maxValue = float(input("Nhập giá trị max: "))
result = find_orders_within_range(df, minValue, maxValue)
print('Danh sách các hóa đơn trong phạm vi giá trị từ',minValue, 'đến', maxValue, 'là:', result)
